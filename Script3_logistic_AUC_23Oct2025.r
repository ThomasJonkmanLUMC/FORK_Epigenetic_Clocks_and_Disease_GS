...
auc_z <- setNames(vector("list", length(clocks)), clocks)
...
for (disease in dis_list) {
  ...
  if (prop_F > 0.1 & prop_F < 0.9) {
    ...
    for (clock in clocks) {
	  ...
	  auc_z[[clock]][[disease]] <- roc.test(roc_obj, roc_obj2)$statistic
...
auc_z <- as.data.frame(do.call('rbind', auc_z))
...
z <- as.data.frame(lapply(auc_z, unlist))
...
z1 <- z1[,c("disease", setdiff(names(z1), "disease"))]
...
tmp <- cbind(d22[,1:2], d11[,2], d33[,2], d44[,2:3], d55[,2])
names(tmp) <- c("disease","auc_null","auc_full","auc_diff","P_auc_diff","Clock","Z_auc")
auc_results <- tmp[,c(1,6,2:4,7,5)]


hrs <- read.csv("/Cluster_Filespace/Marioni_Group/Riccardo/Christos_17Oct2024/Results/All_Results_23Oct2025.csv")
hrs <- hrs[hrs$model=="full",]


final_out <- merge(hrs, auc_results, by=c("disease","Clock"))

write.csv(final_out, "/Cluster_Filespace/Marioni_Group/Riccardo/Christos_17Oct2024/Results/Disease_HR_AUC_merged_24Oct2025.csv", row.names=F)
